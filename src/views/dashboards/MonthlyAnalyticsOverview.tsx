...
import { getMonthlyAnalyticsOverview } from 'src/store/dashboard'
...
const MonthlyAnalyticsOverview = ({ title, subTitle }: Props) => {
  ...
  const monthlyAnalyticsOverview = useSelector((state: any) => state.dashboard.monthlyAnalyticsOverview)
  const isMobile = useMediaQuery(theme.breakpoints.down('sm'))
  const islgScreen = useMediaQuery(theme.breakpoints.up('xl'))

  useEffect(() => {
    // Dispatch the initial action when the component mounts
    dispatch(getMonthlyAnalyticsOverview())
  ...
  useEffect(() => {
    setTotalAmount(monthlyAnalyticsOverview?.transaction?.TotalAuxvaultAmount)
    let tempsales = 0
    monthlyAnalyticsOverview?.response?.map((data: any) => (tempsales = tempsales + data?.totalsales))
    setTotalSales(tempsales)

    const temp = transactionData
    temp[0].value = fCurrency(monthlyAnalyticsOverview?.transaction?.monthTotalCardAmount || 0)
    temp[1].value = fCurrency(monthlyAnalyticsOverview?.transaction?.monthTotalCashAmount || 0)
    // temp[2].value = monthlyAnalyticsOverview?.transaction?.monthTotalAchAmount || 0
    temp[2].value = fCurrency(monthlyAnalyticsOverview?.transaction?.monthTotalGiftCardAmount || 0)
    settransactionData(temp)

    const tempAnaytic = analyticsData
    tempAnaytic[0].value = monthlyAnalyticsOverview?.transaction?.autopayMonth || 0
    tempAnaytic[1].value = monthlyAnalyticsOverview?.transaction?.failedAutopayMonth || 0
    tempAnaytic[2].value = monthlyAnalyticsOverview?.transaction?.repossessedMonth || 0
    tempAnaytic[3].value = monthlyAnalyticsOverview?.transaction?.chargeOffMonth || 0
    tempAnaytic[4].value = monthlyAnalyticsOverview?.transaction?.outForRepo || 0

    setAnalyticsData(tempAnaytic)

    const tempCallData = callingData
    tempCallData[0].value = monthlyAnalyticsOverview?.transaction?.totalCalls || 0
    tempCallData[1].value = monthlyAnalyticsOverview?.transaction?.totalCallsIn || 0
    tempCallData[2].value = monthlyAnalyticsOverview?.transaction?.totalCallsOut || 0
    tempCallData[3].value = monthlyAnalyticsOverview?.transaction?.totalNoAnswerCall || 0
    tempCallData[4].value = monthlyAnalyticsOverview?.transaction?.totalRejectedCall || 0
    setCallingData(tempCallData)
  }, [monthlyAnalyticsOverview?.transaction])
          ...
          <Grid item xs={12} md={5} lg={3.5}>
            ...
            >
              ...
              <div style={{ display: 'flex', flexGrow: 1, flexDirection: 'column', justifyContent: 'space-between' }}>
                {monthlyAnalyticsOverview?.response?.map((data: any, index: number) => {
                  return (
                    <Box
                      key={index}
                      mt={2}
