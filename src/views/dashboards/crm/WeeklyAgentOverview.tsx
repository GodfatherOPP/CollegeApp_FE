...
import { getWeeklyAgentReport } from 'src/store/dashboard'
...
const WeeklyAgentOverview = ({ title, subTitle }: Props) => {
  ...
  const weeklyAgentReport = useSelector((state: any) => state.dashboard.weeklyAgentReport)
  ...
  const setDashboardData = () => {
    setDetails([
      { label: 'Total Calls', value: weeklyAgentReport?.routeData?.allCallsMade || 0 },
      { label: 'Inbound', value: weeklyAgentReport?.routeData?.allCallsIn || 0 },
      { label: 'Outbound', value: weeklyAgentReport?.routeData?.allCallsOut || 0 },
      { label: 'SMS/Text', value: weeklyAgentReport?.routeData?.totalSms || 0 },
      { label: 'Email', value: weeklyAgentReport?.routeData?.totalEmail || 0 }
    ])
    setStatsDetails([
      { label: 'Total Talk Time', value: `${weeklyAgentReport?.routeData?.allcallTime?.toFixed(2)} Hr.` || `0` },
      { label: 'Answered Calls', value: weeklyAgentReport?.routeData?.allAnsweredCall || 0 },
      { label: 'Not Answered Calls', value: weeklyAgentReport?.routeData?.allNoAnswerCall || 0 },
      { label: 'Rejected Calls', value: weeklyAgentReport?.routeData?.allRejectedCall || 0 },
      { label: 'Current Unread Msg.', value: 0 }
    ])
    const tempCurrAcc = (
      (weeklyAgentReport?.routeData?.currentAccount / weeklyAgentReport?.routeData?.totalIdmsCustomer) *
      100
    ).toFixed()
    const tempstarting = (
      (weeklyAgentReport?.routeData?.mondayCurrentAccount / weeklyAgentReport?.routeData?.totalIdmsCustomer) *
      100
    ).toFixed()
    ...
    setProgressData({
      starting: weeklyAgentReport?.routeData?.mondayCurrentAccount || 0
    })
    setExtraData({
      ptp: weeklyAgentReport?.routeData?.totalPtp || 0,
      totalArrangment: weeklyAgentReport?.routeData?.totalArrangment || 0
    })
    setOptions([
      {
        chart: {
        ...
        labels: ['Current Accounts', weeklyAgentReport?.routeData?.currentAccount],
        colors: [hexToRGBA('#C34A36', 1)],
        states: {
          hover: {
            filter: { type: 'none' }
      ...
      },
      {
        ...
        labels: [
          `Starting ( ${weeklyAgentReport?.routeData?.mondayCurrentAccount || 0} )`,
          weeklyAgentReport?.routeData?.mondayCurrentAccount
        ],
        colors: [hexToRGBA('#C34A36', 1)],
        states: {
          hover: {
  ...
  useEffect(() => {
    const filterData = weeklyAgentReport?.data
      ?.map((item: any) => {
...
export default WeeklyAgentOverview
